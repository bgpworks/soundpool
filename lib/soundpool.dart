...
class Soundpool {
  ...
  Soundpool._(
    ...
    StreamType type = StreamType.music,
    int maxStreams = 1,
  ]) : assert(maxStreams > 0),
       _streamType = type,
  ...
  factory Soundpool({
    ...
    options: SoundpoolOptions(streamType: streamType, maxStreams: maxStreams),
  );
  ...
  Future<int> loadUint8List(
    ...
    int soundId = await _platformInstance.loadUint8List(
      poolId,
      rawSound,
      priority,
    );
  ...
  Future<void> setVolume({
    ...
    await _soundpoolId.future.then(
      (poolId) => _platformInstance.setVolume(
        poolId,
        soundId,
        streamId,
        volumeLeft,
  ...
  Future<void> release() async {
    assert(!_disposed, "Soundpool instance was already disposed");
    await _soundpoolId.future.then(
      (poolId) => _platformInstance.release(poolId),
    );
  }
...
class AudioStreamControl {
  ...
  Future setRate({required double playbackRate}) {
    return _pool.setRate(streamId: stream, playbackRate: playbackRate);
  }
}
...
class SoundpoolOptions {
  ...
  Map<String, dynamic> get _platformOptions => Map.fromEntries([
    ...androidOptions.toOptionsMap().entries.map(
      (e) => MapEntry('android_${e.key}', e.value),
    ),
    ...iosOptions.toOptionsMap().entries.map(
      (e) => MapEntry('ios_${e.key}', e.value),
    ),
  ]);
}
